  ...
  return (
    <section id="contact" className="relative py-40 bg-ink-900 overflow-hidden">
      <div
        ...
        style={{
          background: 'radial-gradient(ellipse 60% 60% at 50% 100%, rgba(196,169,110,0.05) 0%, transparent 70%)',
        }}
      />
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <Reveal>
          <h2
            className="font-display font-light text-cream leading-tight mb-8 whitespace-nowrap"
            style={{ fontSize: 'clamp(1.8rem, 5.5vw, 6rem)', letterSpacing: '-0.03em' }}
          >
            Let's build{' '}
            <em
              style={{
        ...
        <Reveal delay={0.15}>
          <p className="font-body font-light text-ink-200 text-base leading-relaxed mb-20 max-w-lg" style={{ lineHeight: 1.8 }}>
            Whether you're looking for a sharp analyst to join your team or a collaborator who can
            turn complex data into clear operational decisions, I'd be glad to connect.
          </p>
        </Reveal>
          ...
          {contactLinks.map((item, i) => (
            <Reveal key={item.label} delay={i * 0.08}>
              <div className="border-b border-ink-800 py-7 grid grid-cols-3 items-center group">
                <span className="font-mono text-[9px] tracking-widest3 uppercase text-ink-400">{item.label}</span>
                {item.href ? (
                  <a
                    href={item.href}
                    target={item.href.startsWith('http') ? '_blank' : undefined}
                    rel="noreferrer"
                    className="col-span-2 font-body text-ink-100 hover:text-gold transition-colors duration-300 flex items-center gap-4"
                  >
                  ...
                  <span className="col-span-2 font-body text-ink-200">{item.value}</span>
                )}
              </div>
            </Reveal>
          ))}
        ...
        <Reveal delay={0.4}>
          <div className="mt-16 flex flex-col sm:flex-row gap-5">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-4 font-body text-xs tracking-widest2 uppercase px-10 py-4 border border-ink-500 text-ink-100 hover:border-gold hover:text-gold transition-all duration-300"
            >
              View LinkedIn
              <span>↗</span>
            </a>
