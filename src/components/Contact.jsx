          ...
          {contactLinks.map((item, i) => (
            <Reveal key={item.label} delay={i * 0.08}>
              <div className="border-b border-ink-800 py-7 grid grid-cols-3 items-center group">
                <span className="font-mono text-xs tracking-widest2 uppercase text-ink-400">{item.label}</span>
                {item.href ? (
                  <a
                    href={item.href}
                    target={item.href.startsWith('http') ? '_blank' : undefined}
                    rel="noreferrer"
                    className="col-span-2 font-body text-sm text-ink-200 hover:text-gold transition-colors duration-300 flex items-center gap-4 group"
                  >
                  ...
                  <span className="col-span-2 font-body text-sm text-ink-300">{item.value}</span>
                )}
              </div>
            </Reveal>
          ))}
